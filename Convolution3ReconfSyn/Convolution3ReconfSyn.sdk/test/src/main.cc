...
XDoconvolution3reconfsyn doConvolutionSyn;
...
int main()		{
	...
	status = initDoConvolution3ReconfSyn(doConvolutionSyn);
	if(status != XST_SUCCESS){
		printf("\rError: DMA init failed\n");
		return XST_FAILURE;
	}
    ...
    XDoconvolution3reconfsyn_Set_inputChannels(&doConvolutionSyn, (int)INPUT_IMG_CHANNELS);
    XDoconvolution3reconfsyn_Set_inputRows(&doConvolutionSyn, (int)IMG_HEIGHT_OR_ROWS);
    XDoconvolution3reconfsyn_Set_inputCols(&doConvolutionSyn, (int)IMG_WIDTH_OR_COLS);
    XDoconvolution3reconfsyn_Set_outputChannels(&doConvolutionSyn, (int)OUTPUT_IMG_CHANNELS);
    XDoconvolution3reconfsyn_Set_relu(&doConvolutionSyn, false);

    XDoconvolution3reconfsyn_Start(&doConvolutionSyn);
