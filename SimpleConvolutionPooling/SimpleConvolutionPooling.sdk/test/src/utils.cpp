...
int initDoPooling(XDosimpleconvolutionpooling &doPooling)     {
    ...
    XDosimpleconvolutionpooling_Config *doPooling_cfg;
    doPooling_cfg =
    		XDosimpleconvolutionpooling_LookupConfig(XPAR_DOSIMPLECONVOLUTIONPOOLING_0_DEVICE_ID);
    if (!doPooling_cfg)
        printf("Error loading config for doConvolutionSyn_cfg\n");

    status = XDosimpleconvolutionpooling_CfgInitialize(&doPooling, doPooling_cfg);
    if (status != XST_SUCCESS)
