...
int initDoConvolutionSyn(XDoconvolutionsyn &doConvolutionSyn)     {
    int status;

    XDoconvolutionsyn_Config *doConvolutionSyn_cfg;
    doConvolutionSyn_cfg = XDoconvolutionsyn_LookupConfig(XPAR_DOCONVOLUTIONSYN_0_DEVICE_ID);
    if (!doConvolutionSyn_cfg)
        printf("Error loading config for doConvolutionSyn_cfg\n");

    status = XDoconvolutionsyn_CfgInitialize(&doConvolutionSyn, doConvolutionSyn_cfg);
    if (status != XST_SUCCESS)
